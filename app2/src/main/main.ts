...
(async () => {
  ...
  let selectedScreenSourceData: { id: string | undefined ; name: string } | undefined
  ...
  const createSourcesWindow = () => {
    ...
    sourcesWindow.on('close', (e) => {
      if (selectedScreenSourceData) {
        appWindow?.webContents.send('send-screen-source-id', selectedScreenSourceData.id, selectedScreenSourceData.name)
      } else if (!isQuitting) {
  ...
  ipcMain.handle('select-screen-source-id', async (_event, id: string | undefined, name: string) => {
    log.info('Screen source selected:', id)
    selectedScreenSourceData = { id, name }
    sourcesWindow?.close()
  })
